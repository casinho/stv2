    ...
    public function handleUpload($uploadDirectory, $name = null){
        ...
        # chunked upload
            ...
            if ($success AND ($totalParts-1 == $partIndex)){
            	...
            	$imagevariables=getimagesize($target);
            	 
            	$imgWidth 	= $imagevariables[0];
            	$imgHeight 	= $imagevariables[1];
            	 
            	if($imgHeight > $this->maxHeight || $imgWidth > $this->maxWidth) {
            		 
            		$img = new EasyImage($target);
            		$img->resize($this->maxWidth, $this->maxHeight);
            		$img->save();            		
            	}
        ...
        # non-chunked upload
            ...
            if ($target){
                ...
                if (move_uploaded_file($file['tmp_name'], $target)){
                	...
                	$this->imgWidth 	= $imagevariables[0];
                	$this->imgHeight 	= $imagevariables[1];
                	
                	$this->extension	= GFunctions::getImageType($imagevariables[2]);
                	
                	if($this->imgHeight > $this->maxHeight || $this->imgWidth > $this->maxWidth) {
...
/*
                	$file = Files::model()->findByAttributes(array('file_hash'=>md5_file($target)));
                	
                	if($file == null) {
                	
	                	$file = new Files();
	                	
	                	$file->file_name 	= $this->uploadName;
	                	$file->uuid			= $uuid;
	                	$file->path 		= $target;
	                	$file->extension 	= $this->extension;
	                	$file->file_hash	= md5_file($target);
	                	$file->user_id		= Yii::app()->user->getId();
	                	$file->datum		= new CDbExpression('NOW()');
	                	$file->image_flag	= 1;
	                	if($file->validate()) {
	                		$file->save();
	                		$this->file_id		= $file->primeryKey;
	                		return array('success'=> true, "uuid" => $file->uuid);
	                	} else {
	                		print_r::pre($file->getErrors());
	                	}
	                	
                	} else {
                		
                		GFunctions::deleteDir(dirname($target));
                		
                		$imagevariables=getimagesize($file->path);
