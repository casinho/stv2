...
class sitemapCommand extends CConsoleCommand {
	...
	public function actionDoMap() {
		...
		try {
			...
			$sitemap = new Sitemap('http://www.santitan.de');
				...
				foreach($model as $kk => $vv) {
					...
					if(isset($vv->datum)) {
						if(isset($vv->datum_only)) {
							$sitemap->addItem($link, $prio, 'weekly', $vv->datum_only);
						} else {
							$sitemap->addItem($link, $prio, 'weekly', $vv->datum);
						}
					} else {
						$sitemap->addItem($link, $prio, 'monthly', date('M Y'));
					}
				}
			}
			...
			$sitemap->createSitemapIndex('http://www.santitan.de/sitemap/', 'Today');
